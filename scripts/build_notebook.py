...
cells.append(code(
    ...
    "# NOT pull a newer torch / torchvision / pillow (the prior approach\n"
    "# pulled torchvision which forced torch 2.10→2.12, and pillow 12 broke\n"
    "# PIL.ImageText). Vision deps are not needed for text-only MCQ work.\n"
    ...
    "# 2. our other libs without cascading either (accelerate / bitsandbytes\n"
    "#    are happy with Colab's torch)\n"
    "_pip('--upgrade', '--no-deps', 'accelerate>=0.34', 'bitsandbytes>=0.43')\n"
    "# 3. plain installs of small libs (no risk to torch/pillow)\n"
    "_pip('scikit-learn', 'matplotlib', 'tqdm', 'datasets', 'nbformat', 'ipywidgets')\n"
    "# 4. transformers' internal deps that we *do* need (huggingface_hub, etc.)\n"
    "#    They typically don't upgrade torch/pillow.\n"
    "_pip('--upgrade', 'huggingface_hub', 'safetensors', 'tokenizers')\n"
    "# 5. Pillow self-heal if a prior --upgrade left it broken (PIL._typing).\n"
    "try:\n"
    "    import PIL.ImageText  # known canary for pillow 12 ABI break\n"
    "except Exception:\n"
    ...
    "if not _has_q35:\n"
    "    print('>>> kernel restart required — the upgraded transformers cannot replace')\n"
    "    print('    the one Colab pre-imported without a fresh kernel. Restarting now.')\n"
    "    import os, IPython\n"
    "    try:\n"
